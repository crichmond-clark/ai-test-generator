...
export default class ParseFile extends Command {
    ...
    static override description = 'this command will take a given file, find all functions and return a new file containing those functions'
    ...
    public async run(): Promise<void> {
        ...
        const outputFilePath = path.join(
            ...
            `extracted_functions_${path.basename(args.file)}`
        );
        const outputFile = project.createSourceFile(outputFilePath);

        const client = new GeminiClient('', JSON.stringify(functionsArray))
